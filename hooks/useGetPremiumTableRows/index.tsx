        ...
        baseExchangeData.map((item) => {
          ...
          const walletLabel = (
            <div className="flex flex-col gap-y-1">
              ...
              <ExchangeNetworkLabel
                exchange={quoteExchange}
                networks={quoteExchangeNetworks}
                fundType="withdraw"
                feeCurrencyPriceKrw={baseExchangePrice}
              />
            </div>
          );
          ...
          const quoteExchangePriceKrw = quoteExchangePrice
            ? BigNumber(quoteExchangePrice).multipliedBy(krwByUsd ?? 0)
            : undefined;
          ...
          const premium = quoteExchangePriceKrw?.gt(0)
            ? BigNumber(baseExchangePrice).minus(quoteExchangePriceKrw).div(quoteExchangePriceKrw).multipliedBy(100)
            : null;
          const premiumLabel = (
            <div className="flex items-center gap-x-1">
              ...
              <div
                className={`inline-flex items-baseline gap-x-0.5 Font_data_16px_num ${premium === null ? 'text-caption' : premium.gt(0) ? 'text-semantic_bull' : 'text-semantic_bear'}`}>
          ...
          const quoteExchangeVolumeKrw = quoteExchangeVolume
            ? BigNumber(quoteExchangeVolume).multipliedBy(krwByUsd ?? 0)
            : undefined;
          const volumeLabel = (
            <div className="flex flex-col items-end gap-y-0.5 text-right Font_data_14px_num">
